...
| **P13 D=300 单admin pool=30**（Stage B B1+B2 后，同 P4b/P12 口径重测） | 3000 | 300→**284.5**（末80s ~274，≈名义95%） | 100 | **0**（status4=0） | - | 未采样 | ~1423 DB往返/s（cron口径5×284.5；P12 真实 cron 7×167≈1169） | 0 | 0 | - | **阶段B验收：决策 2tx→1tx 合并（B1）+ route 0 DB 内存缓存（B2）后同口径密度 166.9→284.5/s（+70%，达名义 ~95%）**；冻结 51775 行/182s、status 全1、distinct=3000、同秒双触发=0、行锁 waits Δ=0、conns 恒31；两轮读数 229→284.5（饱和态 run-to-run 方差，见 §Phase11） |
...
⑤ P12 档"~835~1040 DB往返/s（每触发~5）"中 **5.0 是 probe 手动触发口径**；P12 是 cron 路径，**真实 cron = 7** → DB往返/s ≈ 167×7 ≈ ~1169/s（尾段 ~1456/s）。Stage B（P13）后 cron 口径 = 5。见 P12 档 §7 注脚。
...
- **DB 写放大（成功路径 probe 口径 ~5.0 stmts/触发，df84684 回归 c1-c7 锁定）**：167/s × 5 ≈ **~835 DB往返/s**（尾段 208/s 时 ~1.0k/s），对照 P4b ~1500 DB往返/s @ 150/s——**密度更高而 DB 往返/s 反降近半**。
  - ⚠️ **注脚（§7 修正）**：`tools/probe_write_amp.py` 的 "5.0" 数的是**手动触发**路径（无 claim 事务、无 advanceNextTime），P12 是 **cron** 路径（spec §0 钉死 cron 触发线程热路径 = **7**：claim 锁读1 + advance2 [tx1]；二次锁读3 + count4 + route SELECT5 + INSERT6 [tx2]；touch7 autocommit）。故 P12 档"~835 ≈ 5×167"系手动口径外推，**真实 cron DB往返/s ≈ 167 × 7 ≈ ~1169/s**（尾段 ~208/s 时 ~1456/s）。A/B 的 Δ（10→5 同口径）不受口径差影响，但**绝对数一律以 cron=7 为基线**。Stage B 目标（及 P13 实测）即把 cron 口径压到 **5**（B1 合并单事务 + B2 route 走内存）。
...
### Phase 11 收获（P1 Stage B 写放大收敛后：D=300 单 admin A/B 对照 P12）

**P13 实测（阶段 B 验收门，2026-09-04 14:09:44~14:12:46，182s 冻结窗）。**
同 P12 配方复测：同批 1-3000（loadTestHandler 快任务、K=10、cron 每 10s、全 SINGLE）、单 admin loadtest（3307，8080，Hikari pool=30）、executor 8081、双 fsync 全关。**代码 = HEAD `ad7b789` + 工作区未提交 Stage B**（B1 claim+decide 合并单事务、B2 RegistryCacheService route 内存缓存，ice-ww 自研，commit 待定）。loadtest DB schema 与 dev 逐列一致（已验）。enable 1-3000 打满后 10s settle → 182s 冻结窗，disable 串行停 3000 后即时冻结、0 复活、0 status=0 残留。冻结态 + DB 双口径核验一致：

- **密度：冻结 trigger_time 口径 51775 行 / 182s = 284.5/s**；observe 插入口径 283.9/s（total 7403→59064）；**对照 P12 166.9/s → Δ+70%**。达 D=300 名义密度（300/s）的 ~95%——Stage A 墙由 ~167/s 顶开、本档已逼近 cron 网格本身的名义上限（3000 job × 每 10s）。
- **per-sec：182/182 秒 ≥1（min 46 / max 399 / avg 284.5），零空秒**（对照 P12 min61/max271）。高密度下秒桶起伏更大（max 399 系多 job 秒对齐），无整秒缺口。
- **尾段 80s 均值 273.9/s**（对照 P12 ~208/s）。
- **正确性红线全绿**：status 全 {1: 51775}（100%），status=3=0、status=4=0；**同 job 同秒双触发 = 0**（F6-2 决定性 SQL——B1 合并单事务下 0 双 claim）；distinct job_id = 3000、窗口内 <2 次触发 job = 0。
- **资源**：**Innodb_row_lock_waits Δ=0**（对照 P12 27——合并单事务后单 admin 锁等待归零）；Threads_connected 恒 31（池 30 满配，无 acquire 阻塞信号）。
- **DB 往返/s（cron 口径 5×密度，probe_write_amp_cron 实测锁定 5/触发）**：284.5 × 5 ≈ **1423/s**（对照 P12 真实 cron 7×167≈1169/s、手动口径 835/s——见 P12 档注脚）。**密度 +70% 而 DB 往返/s 仅 +22%（相对真实 cron 基线）** → 写放大按预期随密度下移：Stage B 后每触发 5 条往返、密度贴名义上限，总往返/s 由每触发成本主导转为密度主导。

**F11-1：P1 Stage B（B1+B2）达标判定 + 吞吐对照（工作区代码，commit 待 ice-ww 提交）。**
每触发 DB 往返 **cron 7 → 5**（B1 决策 2tx→1tx 合并 + B2 route 0 DB）、行锁获取 2→1、锁内 registry 读 1→0（回退除外），probe 实测 FU=1.0/core=5.0/route DB≈0/单事务（`probe_write_amp_cron.py` 绿）。同口径密度 **166.9 → 284.5/s（+70%，全窗均值，贴名义上限 ~95%）**；形态由 P12 的"爬升到 ~210/s"到本档全程高密。正确性红线延续（同秒双触发 0、distinct=3000、零空秒、全 success）+ **B2 三专项 PASS**（executor-kill 驱逐、冷启动回退、双 admin F6-2 收敛，均决定性 SQL，详见 spec §6.3 验证记录）。**结论：阶段 B 达标，P1（写放大）两阶段全部完成**——每触发往返 ~10 → 5、决策 2tx→1tx，单 admin 稳态墙由 ~150/s（P4b）→ ~167/s（A）→ **~285/s（B，贴名义上限）**。
诚实标注：（1）非严格单变量 A/B——本档相对 P12 主变量 = B1+B2（同一代码树仅增量这两项），loadtest DB schema 与 dev 逐列一致、环境其余同 P12；（2）**同配方两轮实测 229.2 → 284.5/s**（P13 首轮启停观测 229.2/s +37%、二轮决定性复测 284.5/s +70%）——两轮间仅 checker 边界修正无代码差异，Δ~55/s 落在既有"饱和深度 run-to-run 方差"观测带内（P8b 曾记采样 min34/max200），档值取二轮决定性复测；密度绝对数仅供同架构相对参考（§0 纪律），**阶段 B 相对 Stage A 的增益方向与量级（+37%~+70%）稳健**。
...
统一根因 = **调度写放大**：每触发 3 次独立事务 commit（job_log 插入 / job_info.next_time 更新 / 回调更新）、~10 次 DB 往返 —— 同一杠杆在 fsync 墙、连接池墙、线程池墙、行锁墙下换着方式当瓶颈。**这是 ww-job 最核心的性能话题**：要突破 150/s 级需产品改造（触发线程池/连接池可配化、压缩每触发的 DB 往返、或分片/CAS 去锁），单纯调参只能在当前墙内挪动。**P1（两阶段，2026-09-04）后单 admin 段已更新：Stage A（P12）成功路径每触发 DB 往返 ~10→5、单 admin 稳态墙 ~150→167/s（窗末 ~210/s）；Stage B（P13，B1 合并单事务+B2 route 内存缓存）每触发 cron 口径 7→5、密度 166.9→284.5/s（+70%，贴 300/s 名义上限 ~95%）——墙已由"写放大"顶到 cron 网格名义上限本身。详见 §Phase10 F10-1 + §Phase11 F11-1；双 admin 行锁段与慢任务段结论不变。**
